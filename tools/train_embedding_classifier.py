...
def main() -> None:
    ...
    parser.add_argument("--class-weight", choices=["none", "balanced_sqrt", "balanced"], default="none")
    ...
    model = nn.Linear(x_train.shape[1], len(classes)).to(device)
    class_weight = None
    ...
    torch.save({"state_dict": best_state, "classes": classes, "args": vars(args), "best": best[1]}, args.out_dir / "best_model.pt")
